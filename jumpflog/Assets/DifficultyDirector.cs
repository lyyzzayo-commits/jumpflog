...
public sealed class DifficultyDirector : MonoBehaviour
{
    ...
    // 3) 이미 progress를 계산해서 쓰고 싶을 때(옵션)
    ...
    public int GetTier(float progress) //티어 정하기
    {
    ...
    public float GetLavaSpeed(float progress) //용암 속도 정하기
    {
    ...
    public ObstacleSpawnSettings GetObstacleSettings(float progress) //장애물 세팅
    {
        int tier = GetTier(progress);
        ...
        // progress가 tier3End까지는 점진 증가, 이후는 max(=1)로 고정
    ...
    private DifficultySnapshot EvaluateByProgress(float progress) //DifficultySnapshot 계산
    {
        int tier = GetTier(progress);
    ...
    private float ComposeProgress(float height, float timeSurvived) //progress 계산
    {
    ...
    private static float Remap01(float value, float min, float max) //단순 계산 도우미
    {
        if (max <= min) return 0f;
        return Mathf.Clamp01((value - min) / (max - min));
    }

    // 지금 코드에서는 EaseIn/EaseOut/TierProgress01을 사용하지 않아서 제거해도 됨.
    // 나중에 곡선 난이도(부드러운 증가)를 만들 때 다시 추가하면 됨.
